...
class GenericDataset:
    ...
    def load_data(self):
        self.train_data, self.train_label, self.test_data, self.test_label, self.meta = read_data(self.name)
        return self.train_data, self.train_label, self.test_data, self.test_label, self.meta
...
def backgroud_data_configuration(BACKGROUND_DATA, BACKGROUND_DATA_PERC, dataset):
    ...
    if dataset.train_data is None or dataset.train_label is None or \
       dataset.test_data is None or dataset.test_label is None:
        dataset.load_data()
...
def dataset_mapper(DATASET):
    # Accepts either name or number as string
    if DATASET.isdigit():
        ds_name = number_to_dataset[DATASET]
    else:
        ds_name = DATASET
    return GenericDataset(ds_name)
