...
#
def get_local_weights(input_sample, classifier_model, desired_label, random_state=None):
    """
    Generate a local binary constraint mask for an input time series based on LIMESegment.
    Segments least important to the desired class are allowed to be perturbed (set to 0).

    Args:
        input_sample (ndarray): Shape (T, 1), single time series sample.
        classifier_model (callable): Model returning class probabilities.
        desired_label (int): Target class label for counterfactual generation.
        random_state (int, optional): For reproducibility.

    Returns:
        weighted_steps (ndarray): Shape (1, T, 1), binary mask for time steps.
                                  0 = modifiable (low importance), 1 = constrained (high importance).
    """
    n_timesteps, n_dims = input_sample.shape  # n_dims=1 expected

    # Get segment importance scores for the desired class
    seg_imp, seg_idx = LIMESegment(
        input_sample,
        classifier_model,
        model_type=desired_label,
    ...
    # Identify segments with low contribution to the desired class (bottom 25%)
    masking_threshold = np.percentile(seg_imp, 25)
    masking_idx = np.where(seg_imp <= masking_threshold)

    # Initialize mask: 1 = not modifiable (important), 0 = modifiable
    weighted_steps = np.ones(n_timesteps)

    for start_idx in masking_idx[0]:
        if start_idx < len(seg_idx) - 1:
            weighted_steps[seg_idx[start_idx] : seg_idx[start_idx + 1]] = 0
        elif start_idx < len(seg_idx):
            weighted_steps[seg_idx[start_idx] :] = 0
        else:
            print(f"[Warning] Skipping invalid start_idx={start_idx} (seg_idx length={len(seg_idx)})")
            continue
