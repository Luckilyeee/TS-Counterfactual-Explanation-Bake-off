...
def get_shapelet(X_train, y_train, len_ts):
    ...
    b = int(len_ts * 0.5)
    c = int(len_ts * 0.7)
    st = ShapeletTransform(n_shapelets=250, window_sizes=[a, b, c],
...
def shapelet_category(y_train, indices, label):
    ...
    res = res.astype({"idx": int, "start_point": int, "end_point": int, "label": int})
    res = res.groupby('label')
    res = res.get_group(label).head(7)
    res = np.array(res)
    return res
...
for DS in ['Coffee'
    ...
    for cls in range(nb_classes):
        shapelet_dic1[cls] = xtrain[shapelets[cls][0][0]][shapelets[cls][0][1]:shapelets[cls][0][2]]
        shapelet_dic2[cls] = xtrain[shapelets[cls][1][0]][shapelets[cls][1][1]:shapelets[cls][1][2]]
    ...
    for instance in range(xtest.shape[0]):   #xtest.shape[0]
        ...
        otherclasses_lst = list(shapelet_dic1.keys())
        ...
        for i, c  in enumerate(otherclasses_lst[:1]):
            ...
            len_shape1 = len(shapelet_dic1[c])  # this is to assign the lengths of different shapelets
            len_shape2 = len(shapelet_dic2[c])
            shapelet1 = shapelet_dic1[c].reshape((1, len_shape1, 1))
            shapelet2 = shapelet_dic2[c].reshape((1, len_shape2, 1))
