...
class Counterfactual(Explainer):
    ...
    def __init__(self,
        ...
        with tf.variable_scope('cf_search', reuse=tf.AUTO_REUSE):
            ...
            if not self.model:
                ...
                self.loss_opt = self.loss_pred + 2 * self.loss_dist
    ...
    def explain(self, X: np.ndarray) -> Explanation:
        ...
        # print("Predicted proba is " + str(pred_prob))
    ...
    def _minimize_loss(self,
        ...
        print("One hot of target is "+str(Y_ohe))
        ...
        for l_step in range(self.max_lam_steps):
            ...
            pre_label = np.argmax(pred_p)
            target =np.argpartition(pred_p[0], -2)[-2]
            print("pre label ", pre_label)
            pred_p = self.predict_fn(X_current)
            print("The CF predicted class is " + str(pred_p))
            cf_label = np.argmax(pred_p)
            print("cf label ", cf_label)
            flip = (cf_label != pre_label)
            print("flip: ", flip)

            Target_class_prob = pred_p[0][target]
            print("Target class proba is ", Target_class_prob)


            # adjust the lambda constant via bisection at the end of the outer loop
            # print("Before bisection lam is "+str(lam))
        ...
        print("finish one")
        # plt.style.use('bmh')
        # plt.plot(np.reshape(X, (X.shape[1])), label="Original X", color='magenta')
        # plt.plot(np.reshape(X_current, (X_current.shape[1])), label="Counter Factual", ls='--', color='green')
        ...
        for n in range(1, n_lines + 1):
            plt.plot(np.reshape(X, (X.shape[1]*X.shape[2])),
                     linewidth=2 + (diff_linewidth * n),
                     alpha=alpha_value,
                     color="magenta")
        # plt.title("X and CF")
        # plt.grid(True, 'major', color='k', alpha=0.5)
        # plt.minorticks_on()
        # plt.grid(True, 'minor', 'y', alpha=0.3)
        ...
        self.return_dict['cf'] = np.reshape(X_current, (X_current.shape[1]*X_current.shape[2]))
        self.return_dict['gradient_updates'] = gradient_updates
        self.return_dict['Target_class_prob'] = Target_class_prob
        self.return_dict['flip'] = flip
